...
async def show_cart(update: Update, context: CustomContext):
    ...
    keyboard = [[InlineKeyboardButton(context.words.confirm_order, callback_data='place_order')]]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def place_order(update: Update, context: CustomContext):
    ...
    order = await Order.objects.acreate(bot_user=bot_user, customer_name=bot_user.name, customer_email="", customer_address="")
    ...
    await query.answer(context.words.your_order_accepted, show_alert=True)
